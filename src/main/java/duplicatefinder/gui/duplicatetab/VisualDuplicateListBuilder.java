...
/**
 * Baut die interaktive Gruppen-/Dateiliste für {@link VisualDuplicateDialog}.
 * Struktur und Einschränkungen analog {@link NameCollisionListBuilder}.
 */
final class VisualDuplicateListBuilder {

    private final JDialog dialog;
    private final List<VisualDuplicateGroup> groups;
    private final ExclusionStore exclusions;
    private final Consumer<String> logger;
    private JPanel root;

    VisualDuplicateListBuilder(JDialog dialog, List<VisualDuplicateGroup> groups,
    ...
    private void rebuild() {
        ...
        root.add(Ui.label(groups.size() + " visuelle Gruppe(n) gefunden:", FONT_BOLD, TEXT), at(gbc, row++));

        for (VisualDuplicateGroup group : new ArrayList<>(groups)) {
    ...
    private JPanel buildGroupPanel(VisualDuplicateGroup group) {
        JPanel panel = Ui.panel(new GridBagLayout());
        panel.setBorder(BorderFactory.createCompoundBorder(
                BorderFactory.createLineBorder(BORDER), new EmptyBorder(6, 8, 6, 8)));
        GridBagConstraints gbc = baseConstraints();

        panel.add(Ui.label("Max. Hamming-Distanz: " + group.getMaxHammingDistance(), FONT_BOLD, WARNING),
                at(gbc, 0));
        int r = 1;
    ...
    private void onDelete(Path path) {
        ...
        if (logger != null) logger.accept("Gelöscht (visuelles Duplikat): " + path.getFileName());
